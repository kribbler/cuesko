...
<?php
	...
	function widget($args, $instance) {
		...
		if($this->file_missing_or_old( $username, 30 )){
		...
		if( $haveTwitterData ){
			...
			foreach( $tweets as $tweet ){
				...
				if( !$includeRepliesTF && strlen( $tweet->in_reply_to_screen_name )){ 		continue;	} ?>
					...
					<div class="right"><?php echo $this->fix_twitter_update( $tweet->text, $tweet->entities ); ?><br />
					<?php if( $showTweetTimeTF ): ?>
						<a href="#" class="small">.<?php echo $this->twitter_time_ltw( $tweet->created_at ); ?></a>
					<?php endif; ?>
					</div>
				<?php
				$i++;
	...
	function get_from_twitter( $username ){
		...
		if ($response['code'] == 200 && isset($response['response']) && !empty($response['response'])) {
			...
			update_option('latest_post_widget_'.$username, $response['response']);
			update_option('latest_post_widget_timestamp_'.$username, time());
	...
	function file_missing_or_old( $username, $ageInMinutes ){
		...
		if (time() - (30 * 60) < $latest_post_widget_timestamp)
		{
			return false;
		}
		else
