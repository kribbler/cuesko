...
function ts_get_embaded_video($url) {
	...
	if (strstr($url,'vimeo'))
	{
		...
		{
			return '<iframe src="http://player.vimeo.com/video/'.$matches[0].'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
		}
	}
	else if (strstr($url,'youtu'))
	{
		...
		if (preg_match($pattern, $url, $matches))
		{
			...
			return '<iframe src="http://www.youtube.com/embed/'.$matches[0].'" allowfullscreen></iframe>';
		}
	}
	return '';
}
 ...
 * @since framework 1.0
 */
function ts_get_resized_post_thumbnail($post_id,$size, $alt = '', $class = '', $return_src = false)
{
	$image = wp_get_attachment_url( get_post_thumbnail_id( $post_id ));						
	return ts_get_resized_image_by_size($image, $size, $alt, $class, $return_src);
}
...
//	if (ts_request_filesystem_credentials()) {
		...
		if (file_exists(get_template_directory_uri() . '/css/font-awesome/css/font-awesome.min.css'))
		{
			$pattern = '/\.(icon-(?:\w+(?:-)?)+):before\s+{\s*content:\s*"(.+)";\s+}/';

//			$subject = $wp_filesystem->get_contents(get_template_directory().'/css/font-awesome/css/font-awesome.min.css');
			$subject = file_get_contents(get_template_directory_uri() . '/css/font-awesome/css/font-awesome.min.css');
 ...
 * @return void
 */
function process_form_shortcode() {
	...
	if (count($data) > 0) {
		...
		wp_mail( get_option( 'admin_email' ), 'Form '.get_the_title(get_the_ID()), implode($data,"\n"));
		$shortcode_form_email_sent = true;
	}
}
add_action('init','process_form_shortcode');
 ...
 * @return string
 */
function ts_add_paragraph($content) {
